...
function horizontal(array, index, rowLength, count){
	return isValidRowIndex(index, rowLength, count) ?
		un.reduce(un.range(index, index + count), function(memo, num){
			return memo * array[num];
		}, 1) :
			0;
}

function vertical(array, index, rowLength, count){
	return isValidColumnIndex(index, rowLength, count) ?
		un.reduce(un.range(index, index + (3 * rowLength) + 1, rowLength), function(memo, num){
			return memo * array[num];
		},1) :
			0;
}

function slash(array, index, rowLength, count){
	return isValidRowIndex(index, rowLength, count) && isValidColumnIndex(index, rowLength, count) ?
		un.reduce(un.range(index + 3, index + (3 * rowLength) + 1, rowLength - 1), function(memo, num){
			return memo * array[num];
		},1) :
		0;
}
